<template>
      <div class="card" :class="{teCard : leftData.item.length===5}">
        <div class="rTop">
        ...
        <ul style="flex-wrap: wrap;">
          <li v-for="(item,index) in leftData.item"  v-if="leftData.item.length===5" style="width: 190px;padding: 5px;" class="cu fiveTe" :class="{activeFive : index===4}" @click="toDetial(item)">
            <img :src="item.imgUrl" alt="" :class="{active : !leftData.type  }" style="display: block;width: 100%;height: 190px;">
            <p v-if="leftData.type">{{item.title}}</p>
          </li>
         <li v-for="(item,index) in leftData.item" v-if="leftData.item.length===6" style="width: 190px;padding: 5px;" class="cu" :class="{addmar :index!==5}" @click="toDetial(item)">
          ...
          <li v-for="(item,index) in leftData.item" v-if="leftData.item.length>6" style="width: 190px;padding: 5px;" class="cu teimg" @click="toDetial(item)">
            <img :src="item.imgUrl" alt="" :class="{active : !leftData.type  }" style="display: block;width: 100%;height: 190px;">
            <p v-if="leftData.type">{{item.title}}</p>
          </li>
        </ul>
